...
namespace t265_depth
{
    ...
    void t265Depth::elaborateImages(const std_msgs::Header &header_msg)
    {
        ...
        if (image_left_.rows > 0 && image_right_.rows > 0)
        {
            ...
            undist_image_left_ = undist_image_left_(cv::Range(75,undist_image_left_.rows-150),cv::Range(85, undist_image_left_.cols));
            undist_image_right_ = undist_image_right_(cv::Range(75,undist_image_right_.rows-150),cv::Range(85, undist_image_right_.cols));
            //auto endTime = std::chrono::high_resolution_clock::now();
            //auto compTime = (endTime-startTime);
            //ROS_INFO("Remap Time: ");
            //ROS_INFO_STREAM(compTime.count());    
